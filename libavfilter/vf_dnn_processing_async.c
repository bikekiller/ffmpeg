...
typedef struct DnnProcessingAsyncContext {
    const AVClass *class;
    ...
    int batch_size;
    int already_flushed;
} DnnProcessingAsyncContext;

static int copy_from_frame_to_dnn(DnnProcessingAsyncContext *ctx, const AVFrame *frame, DNNData *dnn_input);
static int copy_from_dnn_to_frame(DnnProcessingAsyncContext *ctx, AVFrame *frame, const DNNData *dnn_output);
static av_always_inline int isPlanarYUV(enum AVPixelFormat pix_fmt);
static int copy_uv_planes(DnnProcessingAsyncContext *ctx, AVFrame *out, const AVFrame *in);

#define OFFSET(x) offsetof(DnnProcessingAsyncContext, x)
#define FLAGS AV_OPT_FLAG_FILTERING_PARAM | AV_OPT_FLAG_VIDEO_PARAM
static const AVOption dnn_processing_async_options[] = {
...
static int activate(AVFilterContext *filter_ctx)
{
    ...
    do {
        ...
        if (ret > 0) {
           if (ctx->async) {
              if ((ctx->dnn_module->execute_model_async)(ctx->model, in, ctx->model_inputname, (const char **)&ctx->model_outputname, 1) != DNN_SUCCESS)
...
static const AVFilterPad dnn_processing_async_inputs[] = {
    {
        .name         = "default",
...
static const AVFilterPad dnn_processing_async_outputs[] = {
    {
        .name = "default",
        .type = AVMEDIA_TYPE_VIDEO,
...
AVFilter ff_vf_dnn_processing_async = {
    .name          = "dnn_processing_async",
    .description   = NULL_IF_CONFIG_SMALL("Apply DNN processing filter to the input asynchronously backend support."),
    .priv_size     = sizeof(DnnProcessingAsyncContext),
    .init          = init,
    .uninit        = uninit,
    .query_formats = query_formats,
    .inputs        = dnn_processing_async_inputs,
    .outputs       = dnn_processing_async_outputs,
    .priv_class    = &dnn_processing_async_class,
    .activate      = activate,
};
